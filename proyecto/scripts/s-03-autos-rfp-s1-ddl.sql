--
-- ER/Studio 8.0 SQL Code Generation
-- Company :      unam.fi
-- Project :      modelo-global.DM1
-- Author :       rho
--
-- Date Created : Thursday, January 14, 2021 13:56:54
-- Target DBMS : Oracle 11g
--

-- 
-- TABLE: PAIS_FI_RFP_S1 
--

CREATE TABLE PAIS_FI_RFP_S1(
    PAIS_ID    NUMBER(4, 0)    NOT NULL,
    NOMBRE     VARCHAR2(40)    NOT NULL,
    CLAVE      VARCHAR2(4)     NOT NULL,
    REGION     VARCHAR2(3)     NOT NULL,
    CONSTRAINT PAIS_PK PRIMARY KEY (PAIS_ID)
)
;



-- 
-- TABLE: SUCURSAL_FI_RFP_S1 
--

CREATE TABLE SUCURSAL_FI_RFP_S1(
    SUCURSAL_ID          NUMBER(10, 0)    NOT NULL,
    NOMBRE               VARCHAR2(40)     NOT NULL,
    SUCURSAL_ANEXA_ID    NUMBER(10, 0),
    CLAVE                VARCHAR2(8)      NOT NULL,
    PAIS_ID              NUMBER(4, 0)     NOT NULL,
    CONSTRAINT SUCURSAL_PK PRIMARY KEY (SUCURSAL_ID), 
    CONSTRAINT RefPAIS_FI_RFP_S11 FOREIGN KEY (PAIS_ID)
    REFERENCES PAIS_FI_RFP_S1(PAIS_ID)
)
;



-- 
-- TABLE: STATUS_AUTO 
--

CREATE TABLE STATUS_AUTO(
    STATUS_AUTO_ID    NUMBER(2, 0)    NOT NULL,
    CLAVE             VARCHAR2(20)    NOT NULL,
    DESCRIPCION       VARCHAR2(40)    NOT NULL,
    CONSTRAINT STATUS_AUTO_PK PRIMARY KEY (STATUS_AUTO_ID)
)
;



-- 
-- TABLE: MARCA_R_RFP_S1 
--

CREATE TABLE MARCA_R_RFP_S1(
    MARCA_ID       NUMBER(3, 0)     NOT NULL,
    CLAVE          VARCHAR2(50)     NOT NULL,
    DESCRIPCION    VARCHAR2(500)    NOT NULL,
    ACTIVO         NUMBER(1, 0)     NOT NULL,
    CONSTRAINT MARCA_PK PRIMARY KEY (MARCA_ID)
)
;



-- 
-- TABLE: MODELO_R_RFP_S1 
--

CREATE TABLE MODELO_R_RFP_S1(
    MODELO_ID      NUMBER(10, 0)    NOT NULL,
    CLAVE          VARCHAR2(50)     NOT NULL,
    DESCRIPCION    VARCHAR2(500)    NOT NULL,
    ACTIVO         NUMBER(1, 0)     NOT NULL,
    MARCA_ID       NUMBER(3, 0)     NOT NULL,
    CONSTRAINT MODELO_PK PRIMARY KEY (MODELO_ID), 
    CONSTRAINT RefMARCA_R_RFP_S17 FOREIGN KEY (MARCA_ID)
    REFERENCES MARCA_R_RFP_S1(MARCA_ID)
)
;



-- 
-- TABLE: AUTO_F1_RFP_S1 
--

CREATE TABLE AUTO_F1_RFP_S1(
    AUTO_ID           NUMBER(10, 0)    NOT NULL,
    ANIO              NUMBER(4, 0)     NOT NULL,
    NUM_SERIE         VARCHAR2(20)     NOT NULL,
    TIPO              CHAR(1)          NOT NULL,
    PRECIO            NUMBER(9, 2)     NOT NULL,
    FECHA_STATUS      DATE             NOT NULL,
    SUCURSAL_ID       NUMBER(10, 0)    NOT NULL,
    STATUS_AUTO_ID    NUMBER(2, 0)     NOT NULL,
    MODELO_ID         NUMBER(10, 0)    NOT NULL,
    CONSTRAINT AUTO_PK PRIMARY KEY (AUTO_ID), 
    CONSTRAINT RefSUCURSAL_FI_RFP_S13 FOREIGN KEY (SUCURSAL_ID)
    REFERENCES SUCURSAL_FI_RFP_S1(SUCURSAL_ID),
    CONSTRAINT RefSTATUS_AUTO4 FOREIGN KEY (STATUS_AUTO_ID)
    REFERENCES STATUS_AUTO(STATUS_AUTO_ID),
    CONSTRAINT RefMODELO_R_RFP_S18 FOREIGN KEY (MODELO_ID)
    REFERENCES MODELO_R_RFP_S1(MODELO_ID)
)
;



-- 
-- TABLE: AUTO_CARGA_F1_RFP_S1 
--

CREATE TABLE AUTO_CARGA_F1_RFP_S1(
    AUTO_ID             NUMBER(10, 0)    NOT NULL,
    PESO_MAXIMO         NUMBER(10, 2)    NOT NULL,
    VOLUMEN             NUMBER(10, 2)    NOT NULL,
    TIPO_COMBUSTIBLE    CHAR(1)          NOT NULL,
    CONSTRAINT AUTO_CARGA_PK PRIMARY KEY (AUTO_ID), 
    CONSTRAINT RefAUTO_F1_RFP_S113 FOREIGN KEY (AUTO_ID)
    REFERENCES AUTO_F1_RFP_S1(AUTO_ID)
)
;



-- 
-- TABLE: AUTO_PARTICULAR_F1_RFP_S1 
--

CREATE TABLE AUTO_PARTICULAR_F1_RFP_S1(
    AUTO_ID          NUMBER(10, 0)    NOT NULL,
    NUM_CILINDROS    NUMBER(1, 0)     NOT NULL,
    NUM_PASAJEROS    NUMBER(1, 0)     NOT NULL,
    CLASE            CHAR(1)          NOT NULL,
    CONSTRAINT AUTO_PARTICULAR_PK PRIMARY KEY (AUTO_ID), 
    CONSTRAINT RefAUTO_F1_RFP_S112 FOREIGN KEY (AUTO_ID)
    REFERENCES AUTO_F1_RFP_S1(AUTO_ID)
)
;



-- 
-- TABLE: PAGO_AUTO_F1_RFP_S1 
--

CREATE TABLE PAGO_AUTO_F1_RFP_S1(
    CLIENTE_ID     NUMBER(10, 0)    NOT NULL,
    AUTO_ID        NUMBER(10, 0)    NOT NULL,
    FECHA_PAGO     DATE             NOT NULL,
    IMPORTE        NUMBER(8, 2)     NOT NULL,
    RECIBO_PAGO    BLOB             NOT NULL,
    CONSTRAINT PAGO_AUTO_PK PRIMARY KEY (CLIENTE_ID, AUTO_ID), 
    CONSTRAINT RefAUTO_F1_RFP_S114 FOREIGN KEY (AUTO_ID)
    REFERENCES AUTO_F1_RFP_S1(AUTO_ID)
)
;



