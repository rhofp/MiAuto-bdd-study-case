...
--                  pago_suscriptor para el nodo 2
  ...
  case 
    when inserting then
      if :new.num_pago >= 1 and :new.num_pago <= 60 then 
        -- Se inserta de manera remota dado que estamos en n2

        -- Haciendo uso de la tabla temporal
        insert into t_pago_suscriptor_insert(num_pago,suscriptor_id,fecha_pago,
          importe, recibo_pago)
        values(:new.num_pago,:new.suscriptor_id,:new.fecha_pago,:new.importe, 
          :new.recibo_pago);

        -- Insercion en el sitio remoto por medio de la tabla temporal 
        ...
        -- Se inserta de manera local dado que estamos en n2
        insert into pago_suscriptor_2(num_pago,suscriptor_id,fecha_pago,importe, 
          recibo_pago)
        values(:new.num_pago,:new.suscriptor_id,:new.fecha_pago,:new.importe, 
          :new.recibo_pago);
