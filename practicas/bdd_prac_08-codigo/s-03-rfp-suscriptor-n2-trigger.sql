...
instead of insert or delete or update on suscriptor
declare
  v_count number;
begin
  case 
    when inserting then
      ...
      -- NO olvidar que siempre se inserta en suscriptor_1
				if v_count > 0 then
      ...
      -- Se revisa pais_1 (frag remoto) y si se cumple se inserta en fragmento
      -- remoto
      else 
        -- Probar en pais_1
        select count(*) into v_count
    ...
    when deleting then
          ...
          if substr(:old.ap_paterno,1,1) between 'A' and 'M' then 
            ...
            -- Borrar en suscriptor_4 (Fragmento LOCAL)
