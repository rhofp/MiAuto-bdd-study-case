...
--@Descripción:     Carga y Export de archivos PDF en el sitio EFMBD_S1
...
--efmd_s1
Prompt conectando a efmbd_s1 como SYS para crear objetos tipo directory
accept p_sys_password default 'system' prompt 'Password de sys [system]: ' hide
connect sys@efmbd_s1/&p_sys_password as sysdba
...
--efmbd_s2
Prompt conectando a efmbd_s2 como SYS para crear objetos tipo directory
accept p_sys_password default 'system' prompt 'Password de sys [system]: ' hide
connect sys@efmbd_s2/&p_sys_password as sysdba

Prompt creando un objeto DIRECTORY para acceder al directorio /tmp/bdd/p06
create or replace directory tmp_dir as '&p_pdf_path';
grant read,write on directory tmp_dir to editorial_bdd;

------------------ Cargando datos en efmbd_s1 ----------------------

Prompt conectando a efmbd_s1 con usuario editorial_bdd para cargar datos binarios
connect editorial_bdd/editorial_bdd@efmbd_s1
/*
* En este sitio se cargarán los siguientes archivos.
* F_EFM_PAGO_SUSCRIPTOR_1
...
Prompt cargando datos binarios en efmbd_s1

begin
  carga_blob_en_bd('TMP_DIR','m_archivo_3.pdf','f_efm_pago_suscriptor_1',
...
Prompt para f_efm_pago_suscriptor_1:
...
begin
  guarda_blob_en_archivo('TMP_DIR','m_export_archivo_3.pdf',
    'f_efm_pago_suscriptor_1','recibo_pago','num_pago','1','suscriptor_id','1');
end;
/
...
------------------ Cargando datos en efmbd_s2 ----------------------

Prompt conectando a efmbd_s2 con usuario editorial_bdd para cargar datos binarios
...
begin
  carga_blob_en_bd('TMP_DIR','m_archivo_1.pdf','f_efm_articulo_1',
  'pdf','articulo_id','1', null, null);

  carga_blob_en_bd('TMP_DIR','m_archivo_2.pdf','f_efm_articulo_1',
  'pdf','articulo_id','2', null, null);

  carga_blob_en_bd('TMP_DIR','m_archivo_4.pdf','f_efm_pago_suscriptor_2',
...
Prompt para f_efm_articulo_1:
select articulo_id,dbms_lob.getlength(pdf) as longitud
from f_efm_articulo_1;

Prompt para f_efm_pago_suscriptor_2:
select num_pago,suscriptor_id,dbms_lob.getlength(recibo_pago) as longitud
...
begin
  guarda_blob_en_archivo('TMP_DIR','m_export_archivo_1.pdf',
    'f_efm_articulo_1','pdf','articulo_id','1',null,null);

  guarda_blob_en_archivo('TMP_DIR','m_export_archivo_2.pdf',
    'f_efm_articulo_1','pdf','articulo_id','2',null,null);

  guarda_blob_en_archivo('TMP_DIR','m_export_archivo_4.pdf',
    'f_efm_pago_suscriptor_2','recibo_pago','num_pago','70','suscriptor_id','2');
end;
/
